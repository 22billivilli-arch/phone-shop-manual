...
function Quiz() {
  ...
  return (
    <div className="space-y-3">
      <label className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold dark:border-slate-800 dark:bg-slate-900">
        <span>🎓 퀴즈 모드 <span className="text-xs font-normal text-slate-400">(정답 가리고 탭하면 공개 · 신입 교육용)</span></span>
        <input
          type="checkbox"
          checked={quizMode}
          onChange={(e) => {
      ...
      <div className="space-y-2.5">
        {manual.quizCases.map((c, i) => {
          const show = !quizMode || revealed[i]
          return (
            <button
              key={i}
              onClick={() => quizMode && toggle(i)}
              className="block w-full rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm dark:border-slate-800 dark:bg-slate-900"
            >
              ...
              <div className="mt-2.5 pl-8">
                {show ? (
                  ...
                  <span className="text-xs font-semibold text-indigo-500">👆 탭하면 정답 공개</span>
                )}
              </div>
            </button>
          )
